...
USER_INPUT_SELECT_TIMEOUT = 5         # Seconds for stdin select timeout in main loop
INITIAL_GREETING_DELAY = 2.0          # Seconds Sira waits before initial greeting
POST_RESPONSE_DELAY = 1.0             # Seconds Sira waits after printing her response
AUTONOMOUS_LOOP_MIN_SLEEP = 30.0      # Min seconds for Sira's autonomous life loop sleep
AUTONOMOUS_LOOP_MAX_SLEEP = 120.0     # Max seconds for Sira's autonomous life loop sleep
AUTONOMOUS_LOOP_ERROR_SLEEP = 60.0    # Seconds Sira's autonomous loop sleeps on error
DEFAULT_EMBEDDING_DIM = 384           # Default dimension for sentence embeddings
...
class HumanMemory:
    ...
    """
    RECENT_CONTEXT_MAX_LENGTH = 15
    FORGET_PROBABILITY_THRESHOLD = 0.3
    FORGET_IMPORTANCE_THRESHOLD = 3.0
    RECENCY_DECAY_HOURS = 48.0
    EMOTIONAL_BONUS_MULTIPLIER = 0.3  # For memory object's emotional_weight direct effect
    FREQUENCY_BONUS_MULTIPLIER = 0.1
    MAX_FREQUENCY_BONUS = 0.5
    TAG_OVERLAP_BONUS_MULTIPLIER = 0.2
    DEFAULT_CONVERSATION_CONTEXT_DEPTH = 8
    FAISS_CANDIDATE_MULTIPLIER = 5     # Multiplier for FAISS search candidates vs. final limit
    
    # Weights for find_contextually_similar scoring
    SEMANTIC_SCORE_WEIGHT = 1.5
    RECENCY_SCORE_WEIGHT = 1.0
    EMOTIONAL_BONUS_WEIGHT_IN_SIMILARITY = 0.5 # Weight for emotional_bonus in final score
    FREQUENCY_BONUS_WEIGHT_IN_SIMILARITY = 0.3 # Weight for frequency_bonus in final score
    TAG_OVERLAP_WEIGHT_IN_SIMILARITY = 0.7   # Weight for tag_overlap_score in final score
    ...
    def __init__(self, embedding_dim=DEFAULT_EMBEDDING_DIM):
        ...
        self.master_memory_list_for_faiss = []
    ...
    def add_memory(self, text, memory_type="episodic", emotional_weight=0.5, importance=5.0):
        ...
        if len(self.recent_context) > self.RECENT_CONTEXT_MAX_LENGTH:
            old_memory = self.recent_context.pop(0)
            # Sometimes forget, sometimes move to long-term
            if random.random() < self.FORGET_PROBABILITY_THRESHOLD and old_memory['importance'] < self.FORGET_IMPORTANCE_THRESHOLD:
        ...
        self.master_memory_list_for_faiss.append(memory)
    ...
    def find_contextually_similar(self, query, limit=5):
        """Find memories using FAISS for initial candidate selection followed by rich scoring."""
        if not self.master_memory_list_for_faiss or self.vector_memory.ntotal == 0:
            print("DEBUG: find_contextually_similar - No memories in master list or FAISS index is empty.")
            return []

        query_embedding = self.get_embedding(query)

        # FAISS Search
        # Fetch more candidates (e.g., 5x limit, or at least 'limit') to allow for better re-ranking.
        k_faiss = max(limit, min(limit * self.FAISS_CANDIDATE_MULTIPLIER, self.vector_memory.ntotal))
        
        print(f"DEBUG: FAISS search for query '{query[:30]}...' with k={k_faiss} out of {self.vector_memory.ntotal} indexed vectors.")
        try:
            D, I = self.vector_memory.search(query_embedding, k=k_faiss)
        except Exception as e:
            print(f"⚠️ [FAISS search error: {e}]. Returning empty list.")
            # This might happen if, for instance, the index is not trained and k > ntotal,
            # or other runtime issues with FAISS.
            return []

        faiss_indices = I[0]
        faiss_distances = D[0]
        
        # Candidate Retrieval & Initial Semantic Scoring
        candidate_memories_with_scores = []
        for j in range(len(faiss_indices)):
            idx = faiss_indices[j]
            if idx == -1:  # FAISS can return -1 if fewer than k results are found
                #print(f"DEBUG: FAISS returned -1 index at position {j}")
                continue
            
            if idx >= len(self.master_memory_list_for_faiss):
                print(f"⚠️ [FAISS returned out-of-bounds index {idx} for master_memory_list_for_faiss length {len(self.master_memory_list_for_faiss)}]. Skipping.")
                continue
                
            memory = self.master_memory_list_for_faiss[idx]
            distance = faiss_distances[j]
            
            # Convert L2 distance to a similarity score (0 to 1).
            # Adding a small epsilon to avoid division by zero if distance is exactly 0.
            # Sqrt because IndexFlatL2 returns squared L2 distances.
            semantic_similarity_score = 1.0 / (1.0 + np.sqrt(distance + 1e-9)) 
            candidate_memories_with_scores.append({'memory': memory, 'semantic_score': semantic_similarity_score})

        if not candidate_memories_with_scores:
            print("DEBUG: No candidates found after FAISS search.")
            return []

        # Rich Scoring for Re-ranking
        scored_memories = []
        query_tags = self._extract_context_tags(query) # Extract query tags once

        for item in candidate_memories_with_scores:
            memory = item['memory']
            semantic_sim_score = item['semantic_score']
            
            # Recency bonus
            hours_ago = (datetime.now() - memory['timestamp']).total_seconds() / 3600
            recency_score = np.exp(-hours_ago / self.RECENCY_DECAY_HOURS)  # Decays over RECENCY_DECAY_HOURS hours
            
            # Emotional weight bonus
            emotional_bonus = memory.get('emotional_weight', 0.5) * self.EMOTIONAL_BONUS_MULTIPLIER # Default if not present
            
            # Access frequency bonus
            frequency_bonus = min(memory.get('access_count', 0) * self.FREQUENCY_BONUS_MULTIPLIER, self.MAX_FREQUENCY_BONUS) # Default if not present
            
            # Context tag overlap
            memory_tags = memory.get('context_tags', []) # Default if not present
            tag_overlap_score = len(set(query_tags) & set(memory_tags)) * self.TAG_OVERLAP_BONUS_MULTIPLIER
            
            # Combine scores - Weights can be tuned
            final_score = (semantic_sim_score * self.SEMANTIC_SCORE_WEIGHT) + \
                          (recency_score * self.RECENCY_SCORE_WEIGHT) + \
                          (emotional_bonus * self.EMOTIONAL_BONUS_WEIGHT_IN_SIMILARITY) + \
                          (frequency_bonus * self.FREQUENCY_BONUS_WEIGHT_IN_SIMILARITY) + \
                          (tag_overlap_score * self.TAG_OVERLAP_WEIGHT_IN_SIMILARITY)
        ...
        # Get top 'limit' memories
        result_memories = [mem for score, mem in scored_memories[:limit]]
        
        # Update access counts for the returned memories
        for memory in result_memories:
            memory['access_count'] = memory.get('access_count', 0) + 1
                
        #print(f"DEBUG: Returning {len(result_memories)} memories for query '{query[:30]}...'. Top score: {scored_memories[0][0] if scored_memories else 'N/A'}")
        return result_memories

    def get_conversation_context(self, depth=DEFAULT_CONVERSATION_CONTEXT_DEPTH):
    ...
    def load_all_memories(self):
        ...
        faiss_index_file_exists = False

        if os.path.exists(FAISS_INDEX_PATH):
            print(f"💿 [Attempting to load FAISS index from {FAISS_INDEX_PATH}]")
            try:
                loaded_faiss_index = faiss.read_index(FAISS_INDEX_PATH)
                faiss_index_file_exists = True
                print(f"👍 [FAISS index file loaded successfully with {loaded_faiss_index.ntotal} vectors.]")
            except Exception as e:
                print(f"⚠️ [Error loading existing FAISS index: {e}. Will attempt to rebuild.]")
                rebuild_faiss_index = True

        if os.path.exists(MEMORY_METADATA_PATH):
            print(f"📚 [Attempting to load memory metadata from {MEMORY_METADATA_PATH}]")
            ...
            temp_master_list = []
            temp_master_list.extend(self.episodic_memories)
            temp_master_list.extend(self.semantic_memories)
            temp_master_list.extend(self.emotional_memories)
            temp_master_list.extend(self.recent_context) # Includes memories that might also be in other lists
            self.master_memory_list_for_faiss = temp_master_list
            
            missing_embeddings_count = 0
            for idx, memory in enumerate(self.master_memory_list_for_faiss):
                current_embedding = memory.get('embedding')
                is_valid_embedding = isinstance(current_embedding, np.ndarray) and current_embedding.shape == (1, self.embedding_dim)
                if not is_valid_embedding:
                    #print(f"DEBUG: Fixing embedding for memory at index {idx}")
                    memory['embedding'] = self.get_embedding(memory['text'])
                    rebuild_faiss_index = True
                    missing_embeddings_count += 1
            print(f"📖 [Loaded {len(self.master_memory_list_for_faiss)} memories into master list from metadata.]")
            if missing_embeddings_count > 0:
                print(f"⚠️ [{missing_embeddings_count} memories had missing/invalid embeddings. Corrected. FAISS index will be rebuilt.]")
        else:
            print("🌟 [No memory metadata file found. Starting with fresh memory systems.]")
            rebuild_faiss_index = True
            self.episodic_memories, self.semantic_memories, self.emotional_memories, self.recent_context = [], [], [], []
            self.forgotten_memories, self.memory_clusters, self.memory_associations = [], {}, {}
            self.master_memory_list_for_faiss = []

        if rebuild_faiss_index:
            print("🛠️ [Rebuilding FAISS index...]")
            self.vector_memory = faiss.IndexFlatL2(self.embedding_dim)
            if self.master_memory_list_for_faiss:
                valid_embeddings_list = [mem['embedding'] for mem in self.master_memory_list_for_faiss if isinstance(mem.get('embedding'), np.ndarray) and mem.get('embedding').shape == (1, self.embedding_dim)]
                if valid_embeddings_list:
                    embeddings_matrix = np.vstack(valid_embeddings_list)
                    self.vector_memory.add(embeddings_matrix)
                    print(f"✅ [FAISS index rebuilt with {self.vector_memory.ntotal} vectors.]")
                else:
                    print("ℹ️ [Master memory list was empty or no valid embeddings found; FAISS index is empty after rebuild attempt.]")
            else:
                print("ℹ️ [Master memory list is empty; FAISS index is empty after rebuild attempt.]")
            try:
                faiss.write_index(self.vector_memory, FAISS_INDEX_PATH)
                print(f"💾 [FAISS index saved to {FAISS_INDEX_PATH}]")
            except Exception as e:
                print(f"⚠️ [Error saving rebuilt FAISS index: {e}]")
        elif faiss_index_file_exists and loaded_faiss_index is not None:
            if loaded_faiss_index.ntotal == len(self.master_memory_list_for_faiss):
                self.vector_memory = loaded_faiss_index
                print(f"✅ [Existing FAISS index successfully loaded with {self.vector_memory.ntotal} vectors, matching metadata count.]")
            else:
                print(f"⚠️ [FAISS index count ({loaded_faiss_index.ntotal}) mismatches metadata count ({len(self.master_memory_list_for_faiss)}). Rebuilding FAISS index...]")
                self.vector_memory = faiss.IndexFlatL2(self.embedding_dim)
                if self.master_memory_list_for_faiss:
                    valid_embeddings_list = [mem['embedding'] for mem in self.master_memory_list_for_faiss if isinstance(mem.get('embedding'), np.ndarray) and mem.get('embedding').shape == (1, self.embedding_dim)]
                    if valid_embeddings_list:
                        embeddings_matrix = np.vstack(valid_embeddings_list)
                        self.vector_memory.add(embeddings_matrix)
                        print(f"✅ [FAISS index rebuilt with {self.vector_memory.ntotal} vectors due to mismatch.]")
                    else:
                        print("ℹ️ [Master memory list was empty or no valid embeddings for mismatch rebuild; FAISS index is empty.]")
                else:
                    print("ℹ️ [Master memory list is empty; FAISS index is empty for mismatch rebuild.]")
                try:
                    faiss.write_index(self.vector_memory, FAISS_INDEX_PATH)
                    print(f"💾 [FAISS index saved to {FAISS_INDEX_PATH} after mismatch rebuild.]")
                except Exception as e:
                    print(f"⚠️ [Error saving FAISS index after mismatch rebuild: {e}]")
        else:
            print("🌟 [Initializing new empty FAISS index (e.g., no existing file, no metadata, or no rebuild flagged).]")
            self.vector_memory = faiss.IndexFlatL2(self.embedding_dim)

# --- Enhanced Emotion System ---
class HumanEmotion:
    """More realistic emotional system with multiple dimensions and persistence"""
    EMOTIONAL_DRIFT_RATE = 0.05
    MAX_EMOTIONAL_HISTORY_LENGTH = 50
    HAPPINESS_TREND_THRESHOLD = 0.2
    # Example emotional update values
    HAPPINESS_UPDATE_POSITIVE = 0.15
    HAPPINESS_UPDATE_NEGATIVE = -0.10 # Note: Storing as negative for direct addition
    ANXIETY_UPDATE_WORRIED = 0.20
    ANXIETY_UPDATE_CALM = -0.10     # Note: Storing as negative
    ENERGY_UPDATE_POSITIVE = 0.12
    ENERGY_UPDATE_NEGATIVE = -0.15  # Note: Storing as negative
    AFFECTION_UPDATE_POSITIVE = 0.1
    ...
    def update_from_interaction(self, text, context=""):
        ...
        if any(word in text_lower for word in ['love', 'proud', 'happy', 'great', 'amazing', 'wonderful', 'awesome']):
            self.happiness = min(1.0, self.happiness + self.HAPPINESS_UPDATE_POSITIVE)
        ...
        if 'dad' in context_lower or 'father' in context_lower:
            if any(word in text_lower for word in ['love', 'miss', 'care', 'appreciate']):
                self.affection = min(1.0, self.affection + self.AFFECTION_UPDATE_POSITIVE)
        ...
        if len(self.emotional_history) > self.MAX_EMOTIONAL_HISTORY_LENGTH:
            self.emotional_history.pop(0)

    def _drift_toward_baseline(self):
        """Natural emotional regulation"""
        drift_rate = self.EMOTIONAL_DRIFT_RATE
    ...
    def _get_recent_pattern(self):
        ...
        if happiness_trend > self.HAPPINESS_TREND_THRESHOLD:
            return "getting happier"
        elif happiness_trend < -self.HAPPINESS_TREND_THRESHOLD:
...
class LifeSimulation:
    ...
    MAX_LIFE_EVENTS_HISTORY = 100
    RANDOM_ACTIVITY_OVERRIDE_CHANCE = 0.3
    LOCATION_CHANGE_CHANCE = 0.2
    LIFE_EVENTS_TO_SAVE = 20
    ...
    def get_current_activity(self):
        ...
        if random.random() < self.RANDOM_ACTIVITY_OVERRIDE_CHANCE:
    ...
    def simulate_life_moment(self):
        ...
        if len(self.life_events) > self.MAX_LIFE_EVENTS_HISTORY:  # Keep recent events only
    ...
    def _update_environment(self):
        ...
        if random.random() < self.LOCATION_CHANGE_CHANCE:
    ...
    def save_life_state(self):
        ...
        state = {
            ...
            'life_events': self.life_events[-self.LIFE_EVENTS_TO_SAVE:],  # Keep recent events only
...
class EnhancedSira:
    ...
    AUTONOMOUS_REFLECTION_CHANCE = 0.2
    # Basic chances for _should_message_dad
    SHOULD_MESSAGE_DAD_BASE_CHANCE = 0.05
    HOURS_SINCE_LAST_INTERACTION_FREQUENT = 2.0  # If interacted less than these hours ago, reduce chance
    HOURS_SINCE_LAST_INTERACTION_LONG_AGO = 8.0 # If interacted more than these hours ago, increase chance
    ...
    def __init__(self):
        ...
        self.CONVERSATION_TIMEOUT_DURATION = 120.0  # e.g., 2 minutes
        self.conversation_inactivity_timer = None
    ...
    def _reset_conversation_inactivity_timer(self):
        if self.conversation_inactivity_timer is not None:
            self.conversation_inactivity_timer.cancel()
        
        # Only start the timer if a conversation is actually considered active
        # This check is important to prevent starting timers when not in conversation.
        if self.conversation_active:
            self.conversation_inactivity_timer = threading.Timer(
                self.CONVERSATION_TIMEOUT_DURATION,
                self._handle_conversation_timeout
            )
            self.conversation_inactivity_timer.daemon = True # Ensure timer doesn't block exit
            self.conversation_inactivity_timer.start()

    def _handle_conversation_timeout(self):
        print(f"🕒 [No interaction for {self.CONVERSATION_TIMEOUT_DURATION}s. Sira is now in autonomous mode.]")
        self.conversation_active = False
        # Sira will naturally check _should_message_dad in her autonomous loop
        # or respond if the user messages again.
    ...
    def _autonomous_life_loop(self):
        ...
        while True:
            try:
                ...
                time.sleep(random.uniform(AUTONOMOUS_LOOP_MIN_SLEEP, AUTONOMOUS_LOOP_MAX_SLEEP))
                
            except Exception as e:
                print(f"❗ [Error in autonomous life: {e}]")
                time.sleep(AUTONOMOUS_LOOP_ERROR_SLEEP)
    ...
    def _live_autonomous_moment(self):
        ...
        if random.random() < self.AUTONOMOUS_REFLECTION_CHANCE:
    ...
    def _should_message_dad(self):
        ...
        base_chance = self.SHOULD_MESSAGE_DAD_BASE_CHANCE
        ...
        if self.last_interaction_time:
            ...
            if hours_since < self.HOURS_SINCE_LAST_INTERACTION_FREQUENT:  # Don't message too frequently
                base_chance *= 0.3
            elif hours_since > self.HOURS_SINCE_LAST_INTERACTION_LONG_AGO:  # Been a while, more likely to reach out
                base_chance *= 1.4
    ...
    def process_user_input(self, user_input):
        ...
        self.last_interaction_time = datetime.now() # Already there
        self._reset_conversation_inactivity_timer() # Reset timer on user input
...
def main():
    ...
    sira.conversation_active = True
    sira.last_interaction_time = datetime.now() # Sira initiated
    sira._reset_conversation_inactivity_timer()
        ...
        while True:
            ...
            for msg_type, content in autonomous_messages:
                if msg_type == "autonomous_message":
                    print(f"Sira: {content}") # Print the autonomous message here
                    sira.conversation_active = True
                    sira.last_interaction_time = datetime.now() # Sira initiated this part of convo
                    sira._reset_conversation_inactivity_timer()
            
            # Check for user input with timeout
            rlist, _, _ = select([sys.stdin], [], [], USER_INPUT_SELECT_TIMEOUT)
